...
*
      Call Invert_(S,Work(ip_S),C,nDim,nDim,Work(ip_B))
*
      Call Free_Work(ip_B)
      Call Free_Work(ip_S)
      Return
      ...
      Subroutine Invert_(S,Eval,C,nDim,nDim2,Blk)
*
*     S: full-storage overlap matrix (it will be destroyed!)
*     C: on exit, the S^-1 matrix
*
      Implicit ReaL*8 (A-H,O-Z)
#include "real.fh"
