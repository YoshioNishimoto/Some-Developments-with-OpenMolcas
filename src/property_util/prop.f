...
*
     ...
     &              +Work(ipPrTot+1)**2
     &              +Work(ipPrTot+2)**2)
       write (*,*) "until I finish the debug"
       write (*,'(f20.10)') work(ipprtot+0)*fact
       write (*,'(f20.10)') work(ipprtot+1)*fact
       write (*,'(f20.10)') work(ipprtot+2)*fact
       write (*,'("factor = ",f20.10)') fact
