...
use External_centers, only: AMP_Center, DMS_Centers, KVectors, nDMS, nEF, nOrdEF, nWel, nXF, OAM_Center, OMQ_Center, XF
...
#   endif
  ...
  if (EMFR) then
    Do k=1,size(KVectors,2)
      ...
      if (abs(temp) > 1.0e-14_wp) then
        temp = (Two*Pi)/temp
      else
        temp = huge(temp)
      end if
      write(LuWr,'(15X,A)') '   Transition moment integrals'
      write(LuWr,'(15X,A,3(F20.8,1X),A)') '   Wavevector k: (',(kVectors(i,k),i=1,3),')'
      write(LuWr,'(15X,A,(F20.8,1X),A)') '   Wavelength:        ',Temp,'a.u.'
      write(LuWr,'(15X,A,(F20.8,1X),A)') '                      ',Temp*Angstrom,'Angstrom'
      write(LuWr,'(15X,A,(F20.8,1X),A)') '                      ',Temp*Angstrom/Ten,'nm'
    end do
  end if
