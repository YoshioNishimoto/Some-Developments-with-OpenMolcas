...
C       end do
C       write (*,'(2i3,3f20.10)') it,iu,vvv,work(lf1+it-1+nasht*(iu-1)),
...
C       write (*,*) "-------------"
C       write (*,*) "icase  = ",icase
C       write (*,*) "nasup  = ", nasup(1,icase)
C       write (*,*) "nisup  = ", nisup(1,icase)
C       write (*,*) "nindep = ", nindep(1,icase)
C       write (*,*) "nexc   = ", nexc(1,icase)
C       write (*,*) "nexces = ", nexces(1,icase)
C     end do
...
C       call test3_dens(work(ipclag))
        write (*,*) "original depsa"
        call sqprt(work(ipdepsa),nasht)
        write (*,*) "original depsa (sym)"
          do i = 1, nasht
          do j = 1, i-1
...
C
        If (NRAS1T+NRAS3T.NE.0) Then
          ...
          !! should be removed...?
        write (*,*) "removing DEPSA of off-diagonal blocks"
        write (*,*) "before"
        call sqprt(work(ipdepsa),nasht)
          Do II = 1, nRAS1T
...
C
        ...
        End If
C       call dscal(25,2.0d+00,work(ipdepsa),1)
C       write (*,*) "2*depsa"
C       call sqprt(work(ipdepsa),5)
C       call dscal(25,0.5d+00,work(ipdepsa),1)
...
C     ! call sqprt(work(ipdepsa),5)
C
C          call dcopy(144,0.0d+00,0,work(ipdpt),1)
...
C     write (*,*) "OLag"
C     do i = 1, 144
C       write (*,'(i3,f20.10)') i,work(ipolag+i-1)
C     end do
C        write (*,*) "dpt2ao ref"
...
C       call dcopy(144,0.0d+00,0,work(ipolag),1)
C       call dcopy(nbast**2,0.0d+00,0,work(ipfptao),1)
...
C    *     + val
        ...
        call dcopy(25,0.0d+00,0,Work(ipWRK1),1)
...
C
        If (IfChol) Then
          ...
          Call DCopy(NumChoTot**2,0.0D+00,0,Work(ipA_PT2),1)
        End If
        Do iSym = 1, nSym
...
C         write (*,*) "ialgo = ", ialgo
     ...
     *                +nSsh(iSym)*nAsh(iSym))*NVLOC_CHOBATCH(1)
       write (*,*) "nChoBuf = ", nchobuf,isym
       write (*,*) nish(isym),nash(isym),nssh(isym),NVLOC_CHOBATCH(1)
            Call GetMem('DENBRA','ALLO','REAL',ipDBra,nChoBuf)
            Call DCopy(nChoBuf,0.0D+00,0,Work(ipDBra),1)
            CALL OLagNS_RI(iSym,Work(ipWRK1),Work(ipWRK2),
     *                     Work(ipDPTC),Work(ipDBra),Work(ipA_PT2),
     *                     NVLOC_CHOBATCH(1))
C           do i = 1, nchobuf
C             write (*,'(i4,f20.10)') i,work(ipdbra+i-1)
C           end do
          Else
          ...
          write (*,*) "OLagNS: CPU/WALL TIME=", cput,wallt
C         write (*,*) "DPT2C"
...
C       write (*,*) "olag after vvvo"
C       call sqprt(work(ipolag),nbast)
C       call abend
          CALL TIMING(CPTF10,CPE,TIOTF10,TIOE)
          CPUT =CPTF10-CPTF0
          WALLT=TIOTF10-TIOTF0
          write (*,*) "OLagVVVO: CPU/WALL TIME=", cput,wallt
          If (IfChol.and.iALGO.eq.1) Then
...
c
c
C       write (*,*) "fptao after olagns"
C       call sqprt(Work(ipfptao),nbast)
C       write (*,*) "fptcao after olagns"
C       call sqprt(Work(ipfptcao),nbast)
C       write (*,*) "olag after olagns"
C       call sqprt(Work(ipolag),nbast)
C
...
C          call test2_dens(work(ipolag),work(ipdepsa))
C       write (*,*) "olag after eigder"
C       call sqprt(Work(ipolag),nbast)
C       write (*,*) "Wlag after eigder"
...
C
     ...
     *                   Work(ipWRK1),Work(ipWRK2))
          ...
          If (NRAS1T+NRAS3T.NE.0) Then
            ...
            End Do
          End If
C         call dcopy(nasht**2,0.0d+00,0,work(ipdepsa),1)
C
...
C       call dcopy(nbast*nbast,0.0d+00,0,Work(ipWRK1),1)
C       do i = 1, 5
...
C         Work(ipWRK1+i-1+nBasT*(i-1)) = 1.0D+00
C       end do
C       write (*,*) "square transformation matrix"
C       call sqprt(work(ipwrk1),12)
C       write (*,*) "OLag before transformation"
C       call sqprt(work(ipolag),12)
C       Do iSym = 1, nSym
C       write (*,*) "olag before"
C       call sqprt(work(ipolag),nbast)
...
C       write (*,*) "olag after antisymetrization"
C       call sqprt(work(ipolag),nbast)

C       write (*,*) "Wlag square"
C       call sqprt(work(ipwlag),nbast)
        !! It seems Molcas does similar to GAMESS
        call daxpy(nbast*nbast,0.5d+00,work(ipolag),1,work(ipwlag),1)
C
...
C       call sqprt(work(ipwlag),nbast)
        !! It seems Molcas does similar to GAMESS
C       call daxpy(nbast*nbast,0.5d+00,work(ipolag),1,work(ipwlag),1)
C
        !! W(MO) -> W(AO) using the quasi-canonical orbitals
        !! No need to back transform to natural orbital basis
        Call DGemm_('N','N',nBasT,nBasT,nBasT,
...
C         end do
C         end do
C         write (*,*) "OLag after antisymmetrization"
C         call sqprt(work(ipolag),12)
C         call abend
          Call DGemm_('N','N',nBasT,nBasT,nBasT,
     *                1.0D+00,Work(ipTrf),nBasT,Work(ipOLag),nBasT,
     ...
     *                1.0D+00,Work(ipWRK1),nBasT,Work(ipTrf),nBasT,
     *                0.0D+00,Work(ipOLag),nBasT)
C       write (*,*) "OLag after transformation (pre)"
C       call sqprt(work(ipolag),12)
C       write (*,*) "trf"
...
C       write (*,*) "RDMEIG after transformation"
C       call sqprt(work(iprdmeig),5)
C       CALL GETMEM('WRK2  ','FREE','REAL',ipWRK2,nBasT*nBasT)
C


C       call dcopy(nbast*nbast,0.0d+00,0,work(ipwlag),1)
C       write (*,*) "Wlag"
C       call sqprt(work(ipwlag),nbast)
C       call test_dens(work(ipolag),work(ipclag),work(iptrf),
C    *                 work(ipwrk1),work(ipwrk2))
...
C         end do
C       end do
C       write (*,*) "clag in det",nclag
C       do i = 1, nclag
C         write (*,'(i3,1f20.10)') i,work(ipclag+i-1)
C       end do
C       write (*,*) "RDMEIG"
C       call sqprt(work(iprdmeig),5)
...
C       call dcopy(ndet,0.0d+00,0,work(ldpt),1)
C       call csdtvc(work(ipclag),work(ldpt),1,
C    *              work(kdtoc),iwork(kicts(1)),1,0)
C       write (*,*) "clag in det",ndet
C       do i = 1, ndet
C         write (*,'(i3,2f20.10)') i,work(ldpt+i-1),work(ipclag+i-1)
C       end do
...
C Scale with 1/DENORM to normalize
      X=1.0D0/DENORM
C     write (*,*) "scaling of DM: ",x
        x = 1.0d+00
      CALL DSCAL_(NDMAT,X,DMAT,1)
C       write (*,*) "final dmat in MO"
C       do i = 1, ndmat
C         write (*,'(i4,f20.10)') i,dmat(i)
C       end do
...
C     write (*,*) "nbast = ", nbast
      Do iSym = 1, nSym
        ...
        NR2   = nRAS2(iSym)
        NR3   = nRAS3(iSym)
        write (*,*) "nBasI",nBas(iSym)
        write (*,*) "nOrbI",nOrb(iSym)
        write (*,*) "nFroI",nFro(iSym)
        write (*,*) "nIshI",nIsh(iSym)
        write (*,*) "nAshI",nAsh(iSym)
        write (*,*) "nSshI",nSsh(iSym)
        write (*,*) "nDelI",nDel(iSym)
        nCor  = nFroI + nIshI
        nOcc  = nCor  + nAshI 
        nVir  = nSshI + nDelI
        ipTrfL = ipTrf + iSQ
...
C     do isym = 1, nsym
C       nbasi = nbas(isym)
C       write (*,*) "for symmetry :", isym,nbasi
...
C
C     write (*,*) "ndref = ", ndref
C     write (*,*) "nstate = ", state
C     write (*,*) "ldref"
C     do i = 1, ndref
C       write (*,'(i3,f20.10)') i,Work(ldref+i-1)
C     end do
C     write (*,*) "ldmix"
C     do istate = 1, nstate
C     write (*,*) "istate = ", istate
C     do i = 1, ndref
C       write (*,'(i3,f20.10)') i,Work(ldmix+i-1+ndref*(istate-1))
C     end do
C     end do
C     write (*,*) "average"
C     do i = 1, ndref
...
C           write (*,'(2i3,f20.10)') it,iu,Work(ipFPT2+iSq-1+iTU)
          End Do
        End Do
        iSQ = iSQ + nOrbI*nOrbI
...
C     write (*,*) "isym,isymi,isymj,isyma,isymb"
C     write (*,*)  isym,isymi,isymj,isyma,isymb
...
C     do i = 1, nasc*nisc
C       write (*,'(i3,2f20.10)') i,work(lstc2+i-1),work(lstc3+i-1)
C     end do
C
      !! D subspace
      nIND  = NINDEP(iSym,5)
...
C           val = val + ddot(nind,work(iptd+nind*(nia-1)),1,
C    *                            work(iptd+nind*(nja-1)),1)
C         end do
C         write (*,'(2i3,f20.10)') ii,ij,val
C       end do
C     end do
C
      !! EP subspace
...
C         write (*,*) "Integrals"
C         do i = 1, nmaxorb**2
C           write (*,'(i4,2f20.10)') i,eri1(i,1),eri2(i,1)
C         end do
C
...
C
      ...
      Call DScal(nOrbT*nOrbT,1.0D+00/DBLE(MAX(1,NACTEL)),DPT2C,1)
C     write (*,*) "olag after virtual derivative"
...
C       write (*,*) "nocc = ", nocc
        Do iSh = 1, nSkal
          ...
          nBasI = iSD(2,iSh)*iSD(3,iSh)
C         write (*,*) isd(2,ish),isd(3,ish),nbasi
C         write (*,*) "ioffao = ", ioffao(ish)
          Do jSh = 1, nSkal
...
C      call sqprt(t_hbf(1,1,ibas0,jbas0),10)
     ...
     *            (T_hbf(i,1,iBas0,jBas0),i=1,nOcc*nOcc)
              End Do
            End Do
C           write (*,*) "nbasi,nbasj = ", nbasi,nbasj
...
C    *              1.0D+00,Work(ipOLAG+nOCC),nOrb(iSymA))
        write (*,*) "olag before vvvo"
        call sqprt(work(ipolag),nbast)
        CALL DGEMM_('T','N',nMaxOrb,nOcc,nBasT,
     *              1.0D+00,Work(LCMOPT2),nBasT,
     *                      Work(ipvLag),nBasT,
     *              1.0D+00,Work(ipOLAG),nOrb(iSymA))
        write (*,*) "olag after vvvo"
        call sqprt(work(ipolag),nbast)
C
        Call GetMem('T2AO','Free','Real',ipT2AO,lT2AO)
C            call dcopy(norbt*norbt,work(ipolag),1,work(ipwrk2),1)
...
C     !     work(ipdpt2+i-1+12*(j-1))=0.0d+00
      ...
      !     write (*,*) "DPT2 recovered"
      !     call sqprt(work(ipdpt2),norbt)
C     !     call dcopy(nbast*nbast,0.0d+00,0,work(ipolag),1)
      !     CALL DGEMM_('N','T',nOrbI,nOrbI,nOrbI,
     ...
     *!                 1.0D+00,Work(ipOLAG),nOrbI)
      !     !! F*D = L^o
      !     !!
      !     write (*,*) "olag after DPT2C"
      !     call sqprt(work(ipolag),nbast)
      !     call dcopy(norbt*norbt,work(ipolag),1,work(ipwrk2),1)
...
C
      ...
      write (*,*) "nfro,nish = ", nfro(isym),nish(isym)
...
C         write (*,'(2i3,2f20.10)') ipq,iab,amat(ipq,iab)
        end do
      end do
C     do inda = 1, nasht
C     do indb = 1, nasht
...
C       write (*,'(4i3,f20.10)') inda,indb,indp,indq,val
C     end do
C     end do
C     end do
C     end do
...
C    *     + depsa(iashi,jashi)*(eps(iorb)-eps(jorb))*0.50d+00
     ...
     *          1.0d+00/(eps(iorb)-eps(jorb))
        End Do
      End Do
      write (*,*) "density square"
      call sqprt(xx,5)
      write (*,*) "antisymmetrized U"
      do i = 1, l1
      write (*,'(i3,f20.10)') i,uu(i)
      end do
      call dcopy(l1*l1,amat,1,asav,1)
      CALL DGETRF(L1,L1,AMat,L1,IPIV,INFO)
      CALL DGETRS('N',L1,1,AMat,L1,IPIV,UU,L1,INFO)
      write (*,*) "answer"
      do i = 1, l1
      write (*,'(i3,f20.10)') i,uu(i)
      end do
      call dcopy(l1*l1,asav,1,amat,1)
      call dgemv('N',l1,l1,
     *           1.0d+00,amat,l1,uu,1,
     *           0.0d+00,xx,1)
      write (*,*) "compare with anti-U"
      do i = 1, l1
      write (*,'(i3,f20.10)') i,xx(i)
      end do
      indpq = 0
      call dcopy(25,xx,1,wrk1,1)
      call dcopy(25,0.0d+00,0,xx,1)
      Do iAshI = 1, nAsh(iSym)
        iOrb = nCorI+iAshI
        indp = iAshI
        Do jAshI = 1, iAshI-1
...
C       End Do
C     End Do
C     write (*,*) "antisymmetrized U"
C     do i = 1, l1
C     write (*,'(i3,f20.10)') i,uu(i)
C     end do
C     CALL DGETRF(L1,L1,AMat2,L1,IPIV,INFO)
C     CALL DGETRS('T',L1,1,AMat2,L1,IPIV,UU,L1,INFO)
C     write (*,*) "answer"
C     do i = 1, l1
C     write (*,'(i3,f20.10)') i,uu(i)
C     end do
C     call dgemv('T',l1,l1,
C    *           1.0d+00,amat2,l1,uu,1,
C    *           0.0d+00,xx,1)
C     write (*,*) "compare with anti-U"
C     do i = 1, l1
C     write (*,'(i3,f20.10)') i,xx(i)
C     end do
...
C     write (*,*) "uu"
C     call sqprt(uu,5)
C       call dgemv('N',l1,l1,
C    *             1.0d+00,amat2,l1,uu,1,
C    *             0.0d+00,xx,1)
C     write (*,*) "xx after DGEMV"
C     call sqprt(xx,5)
C     end do
C     call abend
      pcoeff = 1.234925d-04
C     pcoeff = -6.2567689057d-05
      write (*,*) "pcoeff=",pcoeff
      call dscal(25,pcoeff,g1,1)
      call sqprt(g1,5)
      call dscal(25,sqrt(pcoeff)/pcoeff,g1,1)
      call sqprt(g1,5)
C
