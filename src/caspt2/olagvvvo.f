...
C     write (*,*) "e"
C     write (*,*) "nbasij = ", nbasij, 6*13
C     write (*,*) "keep=",keepi,keepj,keepk,keepl
C     write (*,*) "CMO"
C     call sqprt(cmo,nbast)
C
      !! (ij|kl)
C     write (*,*) "doing actual calculation"
      If (iSymI.EQ.iSymJ .AND. iSymI.EQ.iSymK) Then
C       write (*,*) "aa"
C       write (*,*) "nocc,nbast = ", nocc,nbast
        IOPT=1
        LPQ=0
        IPQ=0
        NPQ=0
...
C     do j = 1, 2
C     do k = 1, 134
C       write (*,'(3i4,f20.10)') i,j,k,brais(i,j,k)
C     end do
C     end do
C     end do
C
...
C     MUL(I,J)=IEOR(I-1,J-1)+1
      ...
      End Do
C
C     write (*,*) "sym=",isymi,isymj,isymk,isyml
      nBasI  = nBas(iSymI)
      KEEPI  = KEEP(iSymI)
      nAuxI  = nAux(iSymI)
      nBasJ  = nBas(iSymJ)
...
C     write (*,*) "nbasij = ", nbasij
      ...
      If (iSymK.EQ.iSymI) iSMax = iSymJ
      iSymL  = 1+iEor(iSymIJ-1,iSymK-1)
C     write (*,*) "isyml,ismax = ", isyml,ismax
      IF (iSymL.GT.iSMax) Return !! should not
      nBasL  = nBas(iSymL)
      KEEPL  = KEEP(iSymL)
      nAuxL  = nAux(iSymL)
      nBasKL = nBasK*nBasL
      IF (iSymK.EQ.iSymL) nBasKL = (nBasK*(nBasK+1))/2
C     write (*,*) "nbaskl = ", nbaskl
      If (nBasKL.eq.0) Return
C
C     write (*,*) "keep=",keepi,keepj,keepk,keepl
      IF (KEEPI+KEEPJ+KEEPK+KEEPL.NE.0) Return ! INTEGRAL BLOCK EXCLUDED BY SETTING KEEP PARAMETERS?
C     write (*,*) "nAux=",nAuxi,nAuxj,nAuxk,nAuxl
      IF (nAuxI+nAuxJ+nAuxK+nAuxL.EQ.0) Return ! frozen orbitals
C
      nIshI = nIsh(iSym)
      nIshJ = nIsh(jSym)
...
c     end do
c     write (*,'(4i3,f20.10)') i,j,k,l,val
c     end do
c     end do
c     end do
c     end do
...
C       do ivec = 134, 1, -1
C           iSwap=0
C           call dcopy(144,0.0d+00,0,work(ipwrk),1)
C           Call Cho_ReOrdr(irc,Work(ip_CHSPC+lscr*(iVec-1)),lscr,jVref,
C    *                      JVEC1,JNUM,NUMV,iSym,JREDC,iSwap,ipWRK,
C    *                      iSkip)
C     if (ivec.eq.1) then
C       do i = 1, 78
C       write (*,'(i3,2f20.10)') i,work(ip_chspc+i-1),work(ipwrk+i-1)
C       end do
C     end if
C          call dcopy(78,work(ipwrk),1,work(ip_chspc+78*(ivec-1)),1)
C       end do
C     nseqij = 0
C     do i = 1, 12
C     do j = 1, i
