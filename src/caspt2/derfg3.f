...
C     do iy=1,nlev
C     do iz=1,nlev
C     write (*,'(4i3,f20.10)') it,iu,iy,iz,g2(it,iy,iu,iz)
C     end do
C     end do
C     end do
C     end do
...
C
      CALL QENTER('DERFG3')
C     call dcopy(nlev**4,0.0d+00,0,df2,1)
...
C     call dcopy(nlev**4,dg2,1,g2,1)
      do ip1=1,nlev2-1
       itlev=idx2ij(1,ip1)
       iulev=idx2ij(2,ip1)
       it=L2ACT(itlev)
...
* bufd: diagonal matrix elements to compute the F matrix
      ...
      if (nbufa.ne.nlev2) then
        write (*,*) "nlev2.ne.nbufa..."
        write (*,*) "need more memory?"
        call abend()
      end if
...
C     write (*,*) "PREP    : CPU/WALL TIME=", cput,wallt
...
C     write (*,*) "before"
C      do i = 1, nsgm1
C        write (*,'(i3,f20.10)') i,work(lbufd+i-1)
C      end do
...
C       write (*,*) "iTask = ", iTask
C       write (*,*) "start,end=",ip1sta,ip1end
C       write (*,*) "ip3mx = ", ip3mx
...
C-position 12345678901234567890
     ...
     &    "---------"
      ...
      if (.false.) then
        !! asdfasdf
          call dcopy(nsgm1,0.0d+00,0,work(lbuf1),1)
...
C     do i = 1, nsgm1
C     write (*,'(i3,f20.10)') i,work(lbuf1+i-1)
C     end do
          call dcopy(nsgm1,0.0d+00,0,work(lbuf1),1)
...
C     end do
     ...
     &         IWORK(LNOCP),IWORK(LIOCP),IWORK(LICOUP),
     &         WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
          call dcopy(nsgm1,0.0d+00,0,work(lbuf2),1)
     ...
     *    write (*,'(4i3,4f20.10)') it,iu,ix,iy,scal,tmp,
     *      g2(it,iu,ix,iy),tmp2
          end do
          end do
        end do
...
C
     ...
     &       IWORK(LNOCP),IWORK(LIOCP),IWORK(LICOUP),
     &       WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
        call dcopy(nsgm1,0.0d+00,0,work(lbuf3),1)
     ...
     &       WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
       ...
       if(iY.eq.iU) then
         tmp=tmp-G2(iV,iX,iT,iZ)
            ...
            tmp2=tmp-g3(ig3)
      if (abs(tmp2).ge.1.0d-10)
     *  write (*,'(6i3,3f20.10)') it,iu,iv,ix,iy,iz,tmp,
     * g3(ig3),tmp2
        end do
...
C       call mktg33(ci)
     ...
     &           WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
        ...
        call dcopy(nlev**4,0.0d+00,0,dg2,1)
      end if
...
C
     ...
     *      + scal*ddot_(nsgm1,ci,1,work(lbuf3),1)
        ...
        call dcopy(nlev**4,0.0d+00,0,dg2,1)
        call dcopy(nlev**4,0.0d+00,0,df2,1)
      end if
...
C     write (*,*) "myBuffer,iTask = ", myBuffer,iTask
...
C       write (*,*) "ip1sta,ip1end=",ip1sta,ip1end
...
C        write (*,'(6i3)') ip1i,itlev,iulev,it,iu,ibuf1
...
C       write (*,*) "ib loop"
        do ib=1,ibuf1
          ...
          it=L2ACT(itlev)
          iu=L2ACT(iulev)
C         write (*,'("itlev,iulev,it,iu = ",4i3)') itlev,iulev,it,iu
          lto=lbuf1+mxci*(ib-1)
C         write (*,'(5f20.10)') (work(lto+i-1),i=1,nsgm1)
C         write (*,'(f20.10)') ddot_(nsgm1,work(lto),1,ci,1)
...
C     CPUT =CPTF10-CPTF0
C     WALLT=TIOTF10-TIOTF0
C     write (*,*) "FG2     : CPU/WALL TIME=", cput,wallt
C     CALL TIMING(CPTF0,CPE,TIOTF0,TIOE)
C
C
C
...
C     CPUT =CPTF10-CPTF0
C     WALLT=TIOTF10-TIOTF0
C     write (*,*) "FG3     : CPU/WALL TIME=", cput,wallt
C
      IF(iPrGlb.GE.DEBUG) THEN
...
C     CPUT =CPTF10-CPTF0
C     WALLT=TIOTF10-TIOTF0
C     write (*,*) "DEPSA(2): CPU/WALL TIME=", cput,wallt
...
C      call dcopy(nlev**4,df2bk,1,df2,1)
C
C     write (*,*) "dg2 after"
C     do it = 1, nlev
C     do iu = 1, nlev
C     do iy = 1, nlev
C     do iz = 1, nlev
C     write (*,'(4i3,f20.10)') it,iu,iy,iz,dg2(it,iu,iy,iz)
C     end do
C     end do
C     end do
C     end do
C       do it = 1, nlev
C       do iu = 1, nlev
C         call dcopy(nsgm1,0.0d+00,0,work(lbuf1),1)
...
C
      ...
      write (*,*) "ng3 = ", ng3
      Do iG3 = 1, NG3
C     write (*,'(i4,2f20.10)') ig3,df3(ig3),dg3(ig3)
        iT=idxG3(1,iG3)
        iU=idxG3(2,iG3)
        iV=idxG3(3,iG3)
        iX=idxG3(4,iG3)
...
C    &         IWORK(LNOCP),IWORK(LIOCP),IWORK(LICOUP),
C    &         WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
C         call dcopy(nsgm1,0.0d+00,0,work(lbuf2),1)
C         do iv = 1, nlev
...
C    &           WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
C         end do
C         write (*,*) it,iu,ddot_(nsgm1,work(lbuf2),1,ci,1)
C       end do
C       end do
C    
     ...
     &!        WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
      ...
      ! !   write (*,'(4i3,f20.10)') it,iu,iy,iz,scal
      ! !   CALL SIGMA1_CP2(IT,IU,SCAL,LSYM,Work(LBUF2),CLAG,
     &! !        IWORK(LNOCSF),IWORK(LIOCSF),IWORK(LNOW),IWORK(LIOW),
     &! !        IWORK(LNOCP),IWORK(LIOCP),IWORK(LICOUP),
     &! !        WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
...
C     !   call dcopy(nsgm1,0.0d+00,0,work(lfcder2),1)
...
C     !     write (*,'(4i3,2f20.10)') it,iu,iy,iz,tmp,tmp2
      !   end do
      !   end do
      ! end do
      ! end do
...
C     call dcopy(nsgm1,0.0d+00,0,work(lbuf1),1)
C     call dcopy(nsgm1,0.0d+00,0,work(lbuf2),1)
C     call dcopy(nsgm1,work(lbufd),1,work(lbuf1),1)
C     do i = 1, nsgm1
...
C       do it = 1, nlev
C       do ix = 1, nlev
C         call dcopy(nsgm1,0.0d+00,0,work(lbufd),1)
...
C     write (*,'(4i3,3f20.10)') it,iu,iv,ix,tmp,tmp2,abs(tmp-tmp2)
      end do
      end do
      end do
      end do
...
C     end do
        ...
        !! <CLAG|Etu|CI>
      ! call dcopy(nlev*nlev,0.0d+00,0,g1,1)
      ! do it = 1, nlev
      ! do iu = 1, nlev
      !   call dcopy(nsgm1,0.0d+00,0,work(lbuf1),1)
      !   CALL SIGMA1_CP2(IT,IU,1.0D+00,LSYM,CI,Work(LBUF1),
     ...
     &!        WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
      ...
      ! end do
      ! end do
      ! write (*,*) "<CLAG|Etu|CI>+<CI|Etu|CLAG>"
      ! call sqprt(g1,nlev)
C
      ! scal = ddot_(nsgm1,ci,1,clag,1)
      ! call daxpy(nsgm1,-scal,ci,1,clag,1)
      ! !! <CLAG|Etu|CI>
      ! call dcopy(nlev*nlev,0.0d+00,0,g1,1)
      ! do it = 1, nlev
      ! do iu = 1, nlev
     ...
     &!        WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
      ...
      ! end do
      ! end do
      ! write (*,*) "<CLAG|Etu|CI>+<CI|Etu|CLAG> after projection"
      ! call sqprt(g1,nlev)
      end if
C     write (*,*) "EtuEyz"
C     do iy = 1, nlev
...
C    &         WORK(LVTAB),IWORK(LMVL),IWORK(LMVR))
C         scal = ddot_(nsgm1,ci,1,work(lbuf2),1)
C         write (*,'(4i3,f20.10)') it,iu,iy,iz,scal
C       end do
C       end do
C     end do
C     end do
...
C        write (*,*) "dg1"
C        call sqprt(dg1,5)
C        call dcopy(25,0.0d+00,0,dg1,1)
C       write (*,*) "after"
C       do i = 1, nsgm1
C         write (*,'(i3,f20.10)') i,work(lbufd+i-1)
C       end do
...
C
C
      write (*,*) "I have not implemented for non-standard Psi0",
     *", when A and C subspaces contribute to the energy, in particular"
      write (*,*)"I cannot debug, because I do not know when it happens"
C     call abend
C
      NLEV2=NLEV**2
      NLEV4=NLEV**4
