...
C     if (iprglb.ne.silent) write (*,*) "skip density in TRDNS2D"
      DO 101 ICASE=1,13
...
C           do i = 1, nin*nis
C             write (*,*) i,work(lg_v2+i-1),work(lg_v1+i-1)
C           end do
...
C             write (*,'(i4,3f20.10)') i,value1,value2,differ
C           end do
          END IF
C         write (*,*) "icase = ", icase
C         write (*,*) "ivec,jvec = ", ivec,jvec
C         write (*,*) "nin,nis,nvec = ", nin,nis,nvec
C         write (*,*) "ind, lg_v1(vec1), lg_v2(vec2)"

C         do i = 1, nin*nis
C           write (*,'(i4,2f20.10)') i,work(lg_v1+i-1),work(lg_v2+i-1)
C         end do
...
C         end if
 ...
 100    CONTINUE
 101  CONTINUE
C     write (*,*) "DPT2 in trdns2d"
C     do i = 1, ndpt2
C       write (*,'(i3,f20.10)') i,dpt2(i)
C     end do
