     ...
     &                               MSTATE(JSTATE)
        ...
        DoPT2NUM = .false.
         If (DerMO.and.iRLXroot.eq.jstate) Then
           IPRGLB = SILENT
           Delta = 1.0d-05
           write (*,*) "Numerical Orbital Lagrangian"
           write (*,*) "norbt,nbast = ", norbt,nbast
           write (*,*) "Delta = ", Delta
...
C
           write (*,*) "Anti-Symmetrized Orbital Lagrangian"
...
C
           write (*,*) "Anti-Symmetrized Orbital Lagrangian/Eigenvalues"
           write (*,*) "This should be DPT2 in MO"
...
C
         ...
         If (DerCI.and.iRLXroot.eq.jstate) Then
           IPRGLB = SILENT
           Delta = 1.0d-06
           write (*,*) "Numerical Configuration Lagrangian"
           write (*,*) "nConf, nRoots = ", nConf,nRoots
           write (*,*) "Delta = ", Delta
...
C              Call dDaFile(LUCIEX,1,Work(ipWRK1),nConf*nRoots,idCI)
             ...
             write (*,'(i5,f20.10)') indCI,Work(ipWRK2+indCI-1)
           End Do
C
C          write (*,*) "Configuration Lagrangian"
C          Do indCI = 1, nConf*nRoots
C            write (*,'(i5,f20.10)') indCI,Work(ipWRK2+indCI-1)
C          End Do
           If (nRoots.ne.1) Then
             write (*,*) "Numerical State Lagrangian when nRoots > 1"
             ijRoots = 0
             Do iRoots = 1, nRoots
     ...
     *                               Work(ipWRK2+nConf*(iRoots-1)),1)
                 write (*,'(2i3,f20.10)') iRoots,jRoots,SLagV
               End Do
             End Do
           End If
C          write (*,*) "After Projection"
C          Do iRoots = 1, nRoots
C            write (*,*) "iRoots = ", iRoots
...
C
         ...
         If (DerGen) Then
           IPRGLB = SILENT
           write (*,*) "Numerical ... Something"
           write (*,*) "TITLE: diagonal B derivative"
           Delta = 1.0d-05
           PT2Delta = Delta
           DoPT2Num = .true.
...
C     write (*,'(i3,3f20.10)') idiffpt2,SLagV, floor(slagv+1.0d-03),
C    *    abs(slagv-floor(slagv+1.0d-03))
             If (DoTriPT2.and.
     *         abs(SLagV-floor(SLagV+1.0d-05)).gt.1.0d-08) Then
               SCAL = SCAL*0.5d+00
               Write (*,'(i3,f20.10," *")') iDiffPT2,SCAL
             Else
               Write (*,'(i3,f20.10)') iDiffPT2,SCAL
             End If
           End Do
         ...
         If (DerICB) Then
           IPRGLB = SILENT
           write (*,*) "Numerical ICB derivative"
           Delta = 1.0d-05
           PT2Delta = Delta
           DoPT2Num = .true.
...
C
     ...
     *                 0.0D+00,Work(ipWRK3),nIN)
           write (*,*) "Orbital Lagrangian in ICB"
...
C
           write (*,*) "Anti-Symmetrized Orbital Lagrangian/Eigenvalues"
           write (*,*) "This should be density in ICB"
