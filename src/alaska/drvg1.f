...
C       Read (LuCMOPT2) (nFro(iSym),iSym=1,8)
        Close (LuCMOPT2)
        write (*,*) "Number of Non-Frozen Occupied Orbitals = ", nOcc(1)
        write (*,*) "Number of     Frozen          Orbitals = ", nFro(1)
C
...
*
     ...
     &               iPrint,iFnc, MemPSO)
         ...
         kBask    = iSD4(3,3)
         lBasl    = iSD4(3,4)
C     write (*,'("iBasi,jBasj,kBask,lBasl = ",4i5)')
C    *   ibasi,jbasj,kbask,lbasl
C     write (*,'("iBasInc,jBasInc,kBasInc,lBasInc = ",4i5)')
...
*
C       write (*,'("iBasn,jBasn,kBasn,lBasn=",4i4)')
C    *    ibasn,jbasn,kbasn,lbasn
C       write (*,'("iFnc(1),iFnc(2),iFnc(3),iFnc(4)=",4i4)')
...
C          write (*,*) "nIJKL, nSO = ", nIJKL, nSO
C         write (*,'(4i3)') ifnc(1)*ibasn,ifnc(2)*jbasn,
C    *                      ifnc(3)*kbasn,ifnc(4)*lbasn
           If (Method_chk.eq.'CASPT2  ') Then
C            write (*,*) "ikS,jlS = ", ikS,jls
...
#endif
C            write (*,'("shell(i,j,k,l) = ", 4i3)') is,js,ks,ls
C      call dcopy(ngrad,0.0d+00,0,temp,1)
           Call TwoEl_g(Coor,
     ...
     &          Aux,nAux,Shijij)
C             do i = 1, 4
C             write (*,'(i3,3f20.10)') i,(temp(j+3*(i-1)),j=1,3)
C             end do
C             write (*,*) "---------------------------"
C     call cpu_time(t8)
C     ttt(6) = ttt(6) + t8-t7
...
C         write (*,'(4i3)') nbasi,nbasj,nbask,nbasl
C     write (*,'(4i3)') (isd4(2,i)*isd4(3,i),i=1,4)
*
...
C         write (*,*) "going to read"
          Read (Unit=LuGamma,Rec=iRec) (WRK1(i),i=1,nOcc*nOcc)
C         if (irec.eq.1) then
C         write (*,*) "the first hbt amplitude"
C          write (*,*) "ibas0,kbas0=",ibas0,kbas0
C          write (*,*) "ibas ,kbas =",ibas,kbas
...
C        write (*,*) "show the AO transformed for ibas,kbas=6,1"
...
C         end if
C      write (*,*) "ibas,kbas = ", ibas0,kbas0
C      do i = 1, nbasj
C      do j = 1, nbasl
C      write (*,'(2i3,f20.10)') i,j,g_toc(i+nbasj*(j-1)+loc)
C      end do
C      end do
        End Do
      End Do
C     write (*,*) "after transformation"
C     do kbas0 = 1, nbask
...
C     If (jS.eq.lS) Then
C     call dcopy(maxshlao**4,0.0d+00,0,g_toc,1)
      Do lBas0 = 1, nBasL
...
C    *  ibas,jbas,kbas,lbas,wrk1(ibas0+nbasi*(kbas0-1))*8.0d+00
      ...
      End Do
C     End If
C     call dscal(maxshlao**4,0.5d+00,g_toc,1)
      Do lBas0 = 1, nBasL
        lBas = iOffAO(lS) + lBas0
        Do iBas0 = 1, nBasI
          iBas = iOffAO(iS) + iBas0
