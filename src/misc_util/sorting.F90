...
#include "compiler_features.h"
    public :: sort, argsort, algorithms, bubble_sort_trsh, swap
#ifndef INTERNAL_PROC_ARG
    public :: compare_int_t
#endif

    type, extends(EnumBase_t) :: tAlgorithm
    end type

    type :: tAlgorithmValues
        type(tAlgorithm) :: &
            mergesort = tAlgorithm(1), &
            quicksort = tAlgorithm(2)
    end type

    type(tAlgorithmValues), parameter :: algorithms = tAlgorithmValues()
    type(tAlgorithm), parameter :: default_algorithm = algorithms%mergesort
...
!> @param[in] algorithm The sorting algorithm to use.
    subroutine sort(V, compare, algorithm)
        ...
        if (algorithm_ == algorithms%mergesort) then
            call mergesort(V, compare)
        else if (algorithm_ == algorithms%quicksort) then
